 ...
 */
@Service
public class WriteOffRuleServiceImpl implements WriteOffRuleService {
    private static final Logger logger = LoggerFactory.getLogger(WriteOffRuleServiceImpl.class);
    @Autowired
    private RuleEparchyParamDao ruleEparchyParamDao;
    @Autowired
    private ItemParamDao itemParamDao;
    @Autowired
    private DepositParamDao depositParamDao;
    @Autowired
    ...
    public void getWriteOffRule(WriteOffRuleInfo writeOffRuleInfo, String eparchyCode, String provinceCode, String netTypeCode) {
        ...
        int ruleId = getRuleEparchy(WriteOffRuleStaticInfo.getAllVRuleeparchy(),
               ActingFeePubDef.ITEM_PRIOR_RULE_TYPE, eparchyCode, provinceCode, netTypeCode);
        if (ruleId > 0) {
        ...
        ruleId = getRuleEparchy(WriteOffRuleStaticInfo.getAllVRuleeparchy(),
               ActingFeePubDef.DEPOSIT_PRIOR_RULE_TYPE, eparchyCode, provinceCode, netTypeCode);
        if (ruleId > 0) {
        ...
        //账本科目限定
        ruleId = getRuleEparchy(WriteOffRuleStaticInfo.getAllVRuleeparchy(),
               ActingFeePubDef.DEPOSIT_LIMIT_RULE_TYPE, eparchyCode, provinceCode, netTypeCode);
        if (ruleId > 0) {
        ...
        //储值方式和帐本类型
        ruleId = getRuleEparchy(WriteOffRuleStaticInfo.getAllVRuleeparchy(),
               ActingFeePubDef.PAYMENT_DEPOSIT_RULE_TYPE, eparchyCode, provinceCode, netTypeCode);
        if (ruleId > 0) {
        ...
        //滞纳金
        ruleId = getRuleEparchy(WriteOffRuleStaticInfo.getAllVRuleeparchy(),
               ActingFeePubDef.LATEFEE_CALCPARA_RULE_TYPE, eparchyCode, provinceCode, netTypeCode);
        if (ruleId > 0) {
        ...
        if (WriteOffRuleStaticInfo.getAllMMCommpara().containsKey(provinceCode)) {
            ...
            if (WriteOffRuleStaticInfo.getAllMMCommpara().get(provinceCode).containsKey(ActingFeePubDef.DEFAULT_EPARCHY_CODE)) {
                writeOffRuleInfo.setProvCommParaMap(WriteOffRuleStaticInfo.getAllMMCommpara().get(provinceCode).get(ActingFeePubDef.DEFAULT_EPARCHY_CODE));
            }
        }
    ...
    private int getRuleEparchy(List<RuleEparchy> ruleList, char ruleType, String eparchyCode, String provinceCode, String netTypeCode) {
        ...
        if (rulePtr == null) {
            //取默认网别规则
            rulePtr = getEparchyRuleInfo(ruleList, ruleType, eparchyCode, provinceCode,ActingFeePubDef.DEFAULT_NET_TYPE_CODE);
            //取默认地市规则
            if (rulePtr == null) {
                rulePtr = getEparchyRuleInfo(ruleList, ruleType,ActingFeePubDef.DEFAULT_EPARCHY_CODE, provinceCode, netTypeCode);
            }
            //取默认地市默认网别规则
            if (rulePtr == null) {
                rulePtr = getEparchyRuleInfo(ruleList, ruleType,ActingFeePubDef.DEFAULT_EPARCHY_CODE, provinceCode,ActingFeePubDef.DEFAULT_NET_TYPE_CODE);
            }
            //取默认省别默认地市规则
            if (rulePtr == null) {
                rulePtr = getEparchyRuleInfo(ruleList, ruleType,ActingFeePubDef.DEFAULT_EPARCHY_CODE,ActingFeePubDef.DEFAULT_PROVINCE_CODE, netTypeCode);
            }
            //全默认优先级最低
            if (rulePtr == null) {
                rulePtr = getEparchyRuleInfo(ruleList, ruleType,ActingFeePubDef.DEFAULT_EPARCHY_CODE,ActingFeePubDef.DEFAULT_PROVINCE_CODE,ActingFeePubDef.DEFAULT_NET_TYPE_CODE);
            }
        }
